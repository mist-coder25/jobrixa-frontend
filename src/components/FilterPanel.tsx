      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 h-full w-80 bg-[#161B22] border-l border-[#30363D] z-[70] flex flex-col shadow-2xl"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#30363D]">
              <div className="flex items-center gap-3">
                <span className="font-bold text-[#E6EDF3] text-lg">Filters</span>
                {activeCount > 0 && (
                  <span className="text-[10px] bg-[#4F8EF7] text-white px-2 py-0.5 rounded-full font-black animate-pulse">
                    {activeCount}
                  </span>
                )}
              </div>
              <button onClick={onClose} className="p-2 text-[#7D8590] hover:text-[#E6EDF3] hover:bg-white/5 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
                        ...
                        <button key={s}
                          ...
                          className={`text-[11px] font-bold px-3 py-1.5 rounded-lg border transition-all ${
                            filters.source.includes(s)
                              ? 'bg-[#3FB950]/10 border-[#3FB950] text-[#3FB950] shadow-[0_0_15px_rgba(63,185,80,0.1)]'
                              : 'bg-[#0D1117] border-[#30363D] text-[#7D8590] hover:border-[#484F58]'
                          }`}>
                          {s}
                        </button>
                    ...
                    <button key={d.value}
                      ...
                      className={`w-full text-left text-xs px-4 py-2.5 rounded-xl transition-all flex items-center justify-between group ${
                        filters.dateRange === d.value
                          ? 'bg-[#1C2128] text-[#E6EDF3] font-bold border border-[#30363D]'
                          : 'text-[#7D8590] hover:bg-[#0D1117] hover:text-[#E6EDF3]'
                      }`}>
                      <span>{d.label}</span>
                      <div className={`w-1.5 h-1.5 rounded-full transition-all ${
                        filters.dateRange === d.value ? 'bg-[#4F8EF7] shadow-[0_0_8px_#4F8EF7]' : 'bg-[#30363D] group-hover:bg-[#484F58]'
                      }`} />
                    </button>
                  ))}
                </div>
                      ...
                      <button key={key}
                        ...
                        className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border transition-all ${
                          filters[key as keyof FilterState]
                            ? 'bg-[#4F8EF7]/10 border-[#4F8EF7]/40 text-[#E6EDF3] font-bold'
                            : 'bg-[#0D1117] border-[#30363D] text-[#7D8590]'
                        }`}>
                        <span className="text-xs">{label}</span>
                        <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${
                          filters[key as keyof FilterState] ? 'bg-[#4F8EF7] border-[#4F8EF7] shadow-lg shadow-[#4F8EF7]/20' : 'border-[#484F58]'
                        }`}>
                          {filters[key as keyof FilterState] && <span className="text-white text-[10px] font-black">✓</span>}
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            <div className="px-6 py-5 border-t border-[#30363D] bg-[#0D1117]/50 backdrop-blur-xl flex gap-3">
              <button 
                onClick={onReset}
                className="flex-1 py-3 border border-[#30363D] text-[#7D8590] hover:text-[#E6EDF3] hover:bg-white/5 rounded-xl text-xs font-bold transition-all"
              >
                Reset
              </button>
              <button 
                onClick={onClose}
                className="flex-1 py-3 bg-[#4F8EF7] hover:bg-[#3B7DE8] text-white rounded-xl text-xs font-black transition-all shadow-lg shadow-[#4F8EF7]/20 hover:scale-[1.02] active:scale-[0.98]"
              >
                Apply
              </button>
            </div>
