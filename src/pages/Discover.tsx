...
const SOURCE_COLORS: Record<string, string> = {
  ...
  default: "bg-border text-[#57606a] border-[#d0d7de]",
};
...
function JobCard({ job, onAddToTracker }: { job: NormalizedJob; onAddToTracker: (job: NormalizedJob) => void }) {
  ...
  return (
    <div className="bg-[#ffffff] border border-[#d0d7de] rounded-xl p-5 flex flex-col gap-4 hover:border-[#4F8EF7]/40 hover:shadow-lg hover:shadow-[#4F8EF7]/5 transition-all cursor-pointer">
      {/* Header */}
      <div className="flex items-start gap-3">
        <CompanyLogo companyName={job.company} size={48} className="border border-[#d0d7de]/50" containerPadding="p-1.5" />
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <h3 className="font-display font-bold text-[#1c2128] text-sm leading-tight truncate">{job.title}</h3>
              <p className="text-xs text-[#57606a] mt-0.5 truncate">{job.company}</p>
            </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        <span className="flex items-center gap-1 px-2.5 py-1 bg-[#ffffff] border border-[#d0d7de] rounded-full text-xs text-[#57606a]">
          {job.location}
        </span>
        {job.isRemote && (
      ...
      {/* Action Buttons */}
      <div className="flex gap-2 pt-1 border-t border-[#d0d7de]">
        <a
          href={job.url}
          target="_blank"
          rel="noopener noreferrer"
          onClick={() => trackEvent('job_viewed', { source: job.source })}
          className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-[#d0d7de] text-[#57606a] hover:text-[#1c2128] hover:border-accent/40 text-xs font-medium transition-all"
        >
        ...
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-[#4F8EF7] hover:bg-[#3B7DE8] text-[#f6f8fa] text-sm font-medium rounded-lg transition-colors"
        >
...
function SkeletonCard() {
  return (
    <div className="bg-[#ffffff] shadow-sm border border-[#d0d7de] rounded-xl p-5 animate-pulse">
...
export default function Discover() {
  ...
  if (!hasAdzuna) {
    return (
      <div style={{color: '#cf222e', padding: '40px', background: '#ffffff', borderRadius: '12px', textAlign: 'center', margin: '20px'}}>
        <h2 style={{fontSize: '18px', fontWeight: 'bold', marginBottom: '8px'}}>Missing Adzuna Credentials</h2>
        <p style={{fontSize: '14px', color: '#57606a'}}>Please add VITE_ADZUNA_APP_ID and VITE_ADZUNA_APP_KEY to your env variables.</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col bg-[#ffffff] overflow-y-auto custom-scrollbar relative">
      <TopBar 
        title="Discover Jobs" 
      ...
      <div className="p-8 space-y-8 max-w-7xl mx-auto w-full">
        ...
        <div className="space-y-4 mt-1">
          <div className="flex gap-3">
            <div className="flex-1 relative group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#57606a] group-focus-within:text-accent transition-colors" />
              <input
                type="text"
                value={inputValue}
                ...
                className="w-full pl-12 pr-4 py-3.5 bg-[#ffffff] shadow-sm border border-[#d0d7de] rounded-xl text-[#1c2128] placeholder:text-[#57606a]/50 text-sm focus:outline-none focus:border-accent focus:shadow-[0_0_0_3px_rgba(108,99,255,0.15)] transition-all"
              />
              {inputValue && (
                <button onClick={() => setInputValue("")} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#57606a] hover:text-[#1c2128]">
                  <X size={14} />
                </button>
              )}
            </div>
            <button
              ...
              disabled={loading}
              className="px-6 py-3.5 bg-accent hover:bg-[#5A52E8] text-[#f6f8fa] rounded-xl text-sm font-semibold flex items-center gap-2 shadow-[0_0_20px_rgba(108,99,255,0.3)] transition-all disabled:opacity-70 shrink-0"
            >
              <Search size={16} />
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative">
              <input
                ...
                className="pl-3 pr-3 py-2 bg-[#ffffff] shadow-sm border border-[#d0d7de] rounded-lg text-xs text-[#1c2128] placeholder:text-[#57606a]/60 focus:outline-none focus:border-accent transition-colors w-36"
              />
            </div>

            <button
               onClick={() => setFilterOpen(true)}
               className="flex items-center gap-1.5 px-3 py-2 bg-[#ffffff] border border-[#d0d7de] rounded-lg text-xs font-semibold text-[#57606a] hover:text-[#1c2128] hover:border-[#4F8EF7]/50 transition-all shadow-sm"
            >
               More Filters
            </button>
            ...
            {(filters.roleType || filters.experience || filters.dateRange !== 'all' || filters.location) && (
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-2 text-xs text-danger border border-[#d0d7de]anger/30 rounded-lg hover:bg-[#f6f8fa]anger/5 transition-colors"
              >
                <X size={10} /> Clear filters
              </button>
            )}

            <span className="ml-auto text-xs text-[#57606a]/60 shrink-0">
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                  searchQuery === cat.id 
                    ...
                    : 'bg-[#ffffff] border-[#d0d7de] text-[#57606a] hover:text-[#1c2128] hover:border-[#4F8EF7]/50'
                }`}
              >
                {cat.label}
              </button>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h3 className="text-xl font-display font-bold text-[#1c2128] mb-2">No jobs found</h3>
            <p className="text-[#57606a] text-sm max-w-xs leading-relaxed">
            ...
            {hasAPIKey && displayedJobs.length > 0 && (
              <div className="flex justify-center pt-8">
                <button
                  ...
                  className="px-8 py-3 bg-[#f6f8fa] border border-[#d0d7de] text-[#1c2128] rounded-xl text-sm font-semibold hover:border-accent/50 hover:bg-accent/5 transition-all disabled:opacity-50 flex items-center gap-2"
                >
                  {loadingMore ? (
                    <>
                      <div className="w-4 h-4 border-[#d0d7de] border-accent border-t-transparent rounded-full animate-spin" />
                      Loading more...
                    </>
                  ) : (
                    <>
