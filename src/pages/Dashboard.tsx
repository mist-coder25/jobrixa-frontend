...
export default function Dashboard() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      trend: <span className="text-xs text-[#8B949E] font-medium">of applied</span>,
  ...
  return (
    <div className="h-full flex flex-col bg-[#0d1117] overflow-y-auto custom-scrollbar">
      ...
      <div className="p-6 md:p-8 space-y-8 animate-in fade-in duration-500">
        ...
        {hasAnyApp === false && (
          <div className="bg-[#0d1117] border border-[#30363D] rounded-xl p-6 mb-6 flex items-center justify-between ">
            <div>
              ...
              <button
                ...
                className="bg-[#58a6ff] hover:bg-[#1f6feb] text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {statCards.map((card, i) => (
            <motion.div
              ...
              className={`bg-[#0d1117] border border-[#30363D] rounded-xl p-5 border-l-2 ${card.borderColor} hover:border-[#58a6ff]/30 transition-colors cursor-default`}
            >
              <div className="flex items-center justify-between mb-3">
                <div className="text-[11px] font-semibold text-[#8B949E] uppercase tracking-wider">
                  {card.label}
                </div>
                <div className="w-8 h-8 rounded-lg bg-[#161B22] flex items-center justify-center text-[#8B949E]">
                  {card.icon}
                </div>
              </div>
              <div className="text-3xl font-bold text-[#C9D1D9] mt-2">
                <AnimatedCounter value={card.value} suffix={card.suffix} />
              </div>
              {card.trend && (
                <div className="text-xs text-[#58a6ff] mt-1 flex items-center">
                  {card.trend}
                </div>
              )}
            </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-[#0d1117]  border border-[#30363D] rounded-xl p-6">
            <h3 className="text-lg font-display font-semibold text-[#C9D1D9] mb-6">Application Activity</h3>
              ...
              {hasAnyApp ? (
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={weeklyData}>
                    ...
                    />
                    <Line type="monotone" dataKey="apps" stroke="#58a6ff" strokeWidth={2} dot={{ fill: '#58a6ff', r: 3 }} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
                <div className="text-center">
                  <div className="w-12 h-12 rounded-full bg-[#161B22] flex items-center justify-center mx-auto mb-3 text-[#586069]">
                    <Target size={20} />
                  </div>
                  <p className="text-sm text-[#8B949E]">No activity data yet</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-[#0d1117]  border border-[#30363D] rounded-xl p-6">
            <h3 className="text-lg font-display font-semibold text-[#C9D1D9] mb-6">Applications by Source</h3>
            <div className="h-[300px] w-full flex items-center justify-center">
              {hasAnyApp ? (
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={sourceData} layout="vertical" margin={{ top: 0, right: 0, left: 10, bottom: 0 }}>
                    ...
                    <XAxis type="number" tick={{ fill: '#8B949E', fontSize: 11 }} axisLine={{ stroke: '#30363D' }} tickLine={false} />
                    <YAxis dataKey="name" type="category" tick={{ fill: '#8B949E', fontSize: 11 }} axisLine={{ stroke: '#30363D' }} tickLine={false} width={80} />
                    <Tooltip 
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-[#8B949E]">No source data yet</p>
                </div>
              )}
            </div>
          </div>
