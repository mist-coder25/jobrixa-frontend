      ...
      <div className="relative w-full max-w-sm bg-[#161B22] border border-[#30363D] rounded-2xl p-8 shadow-2xl shadow-black/50">
        ...
        {step === 'email' && (
          <>
            ...
            <input value={email} onChange={e => setEmail(e.target.value)}
              ...
              className="w-full px-4 py-2.5 bg-[#0D1117] border border-[#30363D] rounded-lg text-[#E6EDF3] text-sm placeholder:text-[#484F58] focus:outline-none focus:border-[#4F8EF7] mb-4 transition-colors" />
            {error && <p className="text-xs text-[#F85149] mb-3">{error}</p>}
            <button onClick={sendOtp} disabled={!email || loading}
              className="w-full py-2.5 bg-[#4F8EF7] hover:bg-[#3B7DE8] text-white font-semibold rounded-lg text-sm disabled:opacity-50 transition-colors">
        ...
        {step === 'otp' && (
          <>
            ...
            <input value={otp} onChange={e => setOtp(e.target.value)}
              ...
              className="w-full px-4 py-2.5 bg-[#0D1117] border border-[#30363D] rounded-lg text-[#E6EDF3] text-sm text-center tracking-widest text-lg font-mono placeholder:text-[#484F58] focus:outline-none focus:border-[#4F8EF7] mb-4 transition-colors" />
            {error && <p className="text-xs text-[#F85149] mb-3">{error}</p>}
        ...
        {step === 'reset' && (
          <>
            ...
            <input value={newPassword} onChange={e => setNewPassword(e.target.value)}
              ...
              className="w-full px-4 py-2.5 bg-[#0D1117] border border-[#30363D] rounded-lg text-[#E6EDF3] text-sm placeholder:text-[#484F58] focus:outline-none focus:border-[#4F8EF7] mb-2 transition-colors" />
            ...
            <button onClick={resetPassword} disabled={newPassword.length < 8 || loading}
              className="w-full mt-4 py-2.5 bg-[#4F8EF7] hover:bg-[#3B7DE8] text-white font-semibold rounded-lg text-sm disabled:opacity-50 transition-colors">
        ...
        {step !== 'done' && (
          <p className="text-center text-xs text-[#7D8590] mt-4">
            Remember it? <span onClick={() => navigate('/login')} className="text-[#4F8EF7] cursor-pointer hover:underline">Sign in</span>
          </p>
        )}
      </div>
    </div>
